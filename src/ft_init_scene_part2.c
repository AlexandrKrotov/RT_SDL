...
void	ft_init_scene_2(t_m *m)
{
	t_rgb		color;
	t_vertex	pos;
	int			size;

	ft_add_light_lst(m->light, (t_vertex){-500, -500, -500}, 0.5);
	ft_add_random_sphere(m);
...
void	ft_init_multilight(t_m *m)
{
	ft_add_light_lst(m->light, (t_vertex){600, 0, -500}, 0.5);
	ft_add_light_lst(m->light, (t_vertex){-600, 0, -500}, 0.5);
	ft_add_plane_ml(m);
	ft_add_cylinder_ml(m);
	ft_add_sphere_ml(m);
}
